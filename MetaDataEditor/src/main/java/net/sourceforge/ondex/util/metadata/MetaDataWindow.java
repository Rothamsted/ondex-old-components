...
public class MetaDataWindow extends JInternalFrame {
	...
	// the corresponding metadata object.
	private ONDEXGraphMetaData md;

	// trees for concept classes, relation types and attribute names
	private METree ccTree, rtTree, anTree;

	// lists for CVs, evidences and units.
	private MEList<DataSource> dataSourceList;
	private MEList<EvidenceType> evList;
	private MEList<Unit> uList;

	// editor panels for all kinds of metadata
	...
	// last used id for an internal frame
	private static int nextId = 0;

	// this frame's id
	private final int id = nextId++;

	// states if the contents have been modified since the last save action
	private boolean modified;

	// the corresponding metadata xml file
	private File file;

	// stack of performed actions and of reverted actions
	private Stack<Operation<?>> actionStack = new Stack<Operation<?>>(),
			reversionStack = new Stack<Operation<?>>();
	...
	public MetaDataWindow(ONDEXGraphMetaData metadata, File file) {
		...
		if (metadata == null) {
			...
			md.createRelationType("r", "is related to", "Root relation type",
					"is related to", false, false, true, false, null);
		} else {
			md = metadata;
		}
	...
	private void formatData() {
		if (md != null) {
			...
			if (ccThing == null) {
				ccThing = md.createConceptClass("Thing", "Thing",
						"All concepts", null);
			}
			...
			if (rtRelated == null) {
				rtRelated = md.createRelationType("r", "is related to",
						"Root relation type", "is related to", false, false,
						true, false, null);
			}
			...
			if (anGDS == null) {
				anGDS = md.createAttributeName("Attribute", "Attribute",
						"General data store", null, Object.class, null);
			}
			...
			if (dataSourceUnknown == null) {
				dataSourceUnknown = md.createDataSource("unknown", "unknown",
						"unknown data source");
			}
			...
			if (usec == null) {
				usec = md
						.createUnit("second", "second", "SI BASE Unit of time");
			}
	...
	private Component setupRelationTypePanel() {
		...
		rtTree.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				if (e.getButton() == MouseEvent.BUTTON3) {
					TreePath path = rtTree.getPathForLocation(e.getX(),
							e.getY());
					if (path != null) {
						rtTree.setSelectionPath(path);
						DefaultMutableTreeNode n = (DefaultMutableTreeNode) path
								.getLastPathComponent();
						boolean root = ((RelationType) n.getUserObject())
								.getId().equals("r");
						MEPopupMenu<RelationType> popup = new MEPopupMenu<RelationType>(
								MetaDataType.RELATION_TYPE, rtTree, n,
								rtEditorPanel, root);
		...
		rtTree.addKeyListener(new KeyAdapter() {
			public void keyTyped(KeyEvent e) {
				...
				if (e.getKeyCode() == KeyEvent.VK_DELETE) {
					...
					// rtTree.getSelectionPath().getLastPathComponent();
					// RelationType rt = (RelationType)node.getUserObject();
					// DefaultMutableTreeNode parent = (DefaultMutableTreeNode)
					// node.getParent();
					// int index = parent.getIndex(node);
					// RemovalOperation<RelationType> a = new
					// RemovalOperation<RelationType>(rt, rtTree, node, index);
					// performOperation(a);
				}
			}
		});
		...
		rtEditorPanel.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				UpdateOperation<RelationType> a = rtEditorPanel
						.extractChangeAction();
				if (a != null) {// can be null when keystroke doesn't change
								// data
					performOperation(a);
					rtTree.getModel().valueForPathChanged(
							rtTree.getSelectionPath(),
							rtEditorPanel.getContents());
				}
			}
		});
	...
	private Component setupConceptClassPanel() {
		...
		ccTree.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				if (e.getButton() == MouseEvent.BUTTON3) {
					TreePath path = ccTree.getPathForLocation(e.getX(),
							e.getY());
					if (path != null) {
						ccTree.setSelectionPath(path);
						DefaultMutableTreeNode n = (DefaultMutableTreeNode) path
								.getLastPathComponent();
						boolean root = ((ConceptClass) n.getUserObject())
								.getId().equals("Thing");
						MEPopupMenu<ConceptClass> popup = new MEPopupMenu<ConceptClass>(
								MetaDataType.CONCEPT_CLASS, ccTree, n,
								ccEditorPanel, root);
		...
		ccEditorPanel.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				UpdateOperation<ConceptClass> a = ccEditorPanel
						.extractChangeAction();
				if (a != null) {// can be null when keystroke doesn't change
								// data
					performOperation(a);
					ccTree.getModel().valueForPathChanged(
							ccTree.getSelectionPath(),
							ccEditorPanel.getContents());
				}
			}
		});
	...
	private Component setupCVPanel() {
		DataSource dataSourceUnknown = md.getDataSource("unknown");
		dataSourceList = new MEList<DataSource>(MetaDataType.CV, md,
				"Data sources");
		dataSourceEditorPanel = new EditorPanel<DataSource>(dataSourceUnknown,
				md);

		dataSourceList.getSelectionModel().addListSelectionListener(
				new ListSelectionListener() {
					@Override
					public void valueChanged(ListSelectionEvent e) {
						if (!(e.getValueIsAdjusting() || dataSourceList
								.getSelectionModel().getMinSelectionIndex() < 0)) {
							int index = dataSourceList
									.convertRowIndexToModel(dataSourceList
											.getSelectionModel()
											.getMinSelectionIndex());
							DataSource dataSource = dataSourceList
									.getMDListModel().getMetaDataAt(index);
		...
		dataSourceList.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				if (e.getButton() == MouseEvent.BUTTON3) {
					...
					dataSourceList.getSelectionModel().setSelectionInterval(
							index, index);
					MEPopupMenu<DataSource> popup = new MEPopupMenu<DataSource>(
							MetaDataType.CV, dataSourceList, index,
		...
		dataSourceEditorPanel.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				UpdateOperation<DataSource> a = dataSourceEditorPanel
						.extractChangeAction();
				if (a != null) {// can be null when keystroke doesn't change
								// data
					performOperation(a);
	...
	private Component setupETPanel() {
		EvidenceType etIMPD = md.getEvidenceType("IMPD");
		evList = new MEList<EvidenceType>(MetaDataType.EVIDENCE_TYPE, md,
				"Evidence types");
		evEditorPanel = new EditorPanel<EvidenceType>(etIMPD, md);

		evList.getSelectionModel().addListSelectionListener(
				new ListSelectionListener() {
					@Override
					public void valueChanged(ListSelectionEvent e) {
						if (!(e.getValueIsAdjusting() || evList
								.getSelectionModel().getMinSelectionIndex() < 0)) {
							int index = evList
									.convertRowIndexToModel(evList
											.getSelectionModel()
											.getMinSelectionIndex());
							EvidenceType ev = evList.getMDListModel()
									.getMetaDataAt(index);
		...
		evList.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				if (e.getButton() == MouseEvent.BUTTON3) {
					...
					evList.getSelectionModel().setSelectionInterval(index,
							index);
					MEPopupMenu<EvidenceType> popup = new MEPopupMenu<EvidenceType>(
							MetaDataType.EVIDENCE_TYPE, evList, index,
							evEditorPanel);
		...
		evEditorPanel.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				UpdateOperation<EvidenceType> a = evEditorPanel
						.extractChangeAction();
				if (a != null) {// can be null when keystroke doesn't change
								// data
					performOperation(a);
					int index = evList.convertRowIndexToModel(evList
							.getSelectionModel().getMinSelectionIndex());
	...
	private Component setupUnitPanel() {
		...
		uList.getSelectionModel().addListSelectionListener(
				new ListSelectionListener() {
					@Override
					public void valueChanged(ListSelectionEvent e) {
						if (!(e.getValueIsAdjusting() || uList
								.getSelectionModel().getMinSelectionIndex() < 0)) {
							int index = uList
									.convertRowIndexToModel(uList
											.getSelectionModel()
											.getMinSelectionIndex());
							Unit u1 = uList.getMDListModel().getMetaDataAt(
									index);
		...
		uList.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				if (e.getButton() == MouseEvent.BUTTON3) {
					...
					uList.getSelectionModel()
							.setSelectionInterval(index, index);
					MEPopupMenu<Unit> popup = new MEPopupMenu<Unit>(
							MetaDataType.UNIT, uList, index, uEditorPanel);
		...
		uEditorPanel.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (a != null) {// can be null when keystroke doesn't change
								// data
					performOperation(a);
					int index = evList.convertRowIndexToModel(uList
							.getSelectionModel().getMinSelectionIndex());
	...
	private Component setupAttributeNamePanel() {
		...
		anTree.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				if (e.getButton() == MouseEvent.BUTTON3) {
					TreePath path = anTree.getPathForLocation(e.getX(),
							e.getY());
					if (path != null) {
						anTree.setSelectionPath(path);
						DefaultMutableTreeNode n = (DefaultMutableTreeNode) path
								.getLastPathComponent();
						boolean root = ((AttributeName) n.getUserObject())
								.getId().equals("Attribute");
						MEPopupMenu<AttributeName> popup = new MEPopupMenu<AttributeName>(
								MetaDataType.ATTRIBUTE_NAME, anTree, n,
								anEditorPanel, root);
		...
		anEditorPanel.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				UpdateOperation<AttributeName> a = anEditorPanel
						.extractChangeAction();
				if (a != null) {// can be null when keystroke doesn't change
								// data
					performOperation(a);
					anTree.getModel().valueForPathChanged(
							anTree.getSelectionPath(),
							anEditorPanel.getContents());
				}
			}
		});
	...
	public void performOperation(Operation<?> a) {
		...
		actionStack.push(a);
		setModified(true);
		MetaDataEditor.getInstance().setActionEnabled(
				new String[] { "Edit", "Undo" }, true);
	}
	...
	public void undo() {
		if (actionStack.size() > 0) {
			...
			updateAll();
			MetaDataEditor.getInstance().setActionEnabled(
					new String[] { "Edit", "Redo" }, true);
			if (actionStack.size() == 0) {
				MetaDataEditor.getInstance().setActionEnabled(
						new String[] { "Edit", "Undo" }, false);
			}
		}
	}
