...
public class InsertOperation<M extends MetaData> extends Operation<M> {
	...
	public InsertOperation(M md, METree tree, DefaultMutableTreeNode parent,
			int index) {
		super(md);
		this.tree = tree;
		this.parent = parent;
	...
	public InsertOperation(M md, MEList<M> list, int index) {
		super(md);
		this.list = list;
		this.index = index;
	}
	...
	public void perform() {
		...
		if (tree != null) {
			...
			index = list.convertRowIndexToView(index);
