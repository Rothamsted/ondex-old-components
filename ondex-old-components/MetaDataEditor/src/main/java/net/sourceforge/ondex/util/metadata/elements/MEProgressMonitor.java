...
/**
 * This is the OVTK's ProgressMonitor class. It can automatically monitor a running process
 * that has implemented the interface "Monitorable".
 * For easier use it also provides several convenience methods to cope with race conditions.
 * Whenever the actual "Monitorable" instance is not accessible before the worker thread has
 * been started, the waitForInitialization() method can be called in the gui thread. In the meantime
 * the worker thread can take its time to initialize everything and then call the notifyInitializationDone()
 * method before starting the real computation.
 * 
 * Consider the following example with a global variable <code> monitorableProcess </code>:
 * 
 * <code> 
 * 
 ...
 * 
 * 
 * If however the Monitorable instance is ready from the start, anyway, then you might just as
 * well just start the worker thread and call the start() method without worrying about sync.
 * 
 * Example:
 * 
 * <code>
 ...
 *
 */
public class MEProgressMonitor extends JDialog implements ActionListener {
	 ...
	 */
	private MEProgressMonitor(String title, final Monitorable process) {
		super(MetaDataEditor.getInstance(), title);
	...
	private void startFeeder() {
		//feeds the monitor continuously with the information it gets from the process
		Thread feederthread = new Thread(getTitle()+" progess feeder") {
			public void run() {
				while (!process.getState().equals(Monitorable.STATE_TERMINAL)) {
					...
					note.setText("   "+process.getState());
					
					if (canceled 
						|| (process.getProgress() >= process.getMaxProgress())
				...
				if (process.getUncaughtException() != null) {
					...
					ErrorDialog.show(false, exc, Thread.currentThread());
				}
				dispose();
			}
		};
	...
	/**
	 * basically calls wait in a synchronzied environment on the monitorable process.
	 * @param m the process.
	 */
	...
	/**
	 * basically calles notifyAll in a synchronized environment on the monitorable process.
	 * @param m the process.
	 */
